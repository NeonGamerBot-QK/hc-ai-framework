...
describe('AiApiWrapper', () => {
    ...
    describe('V1Api', () => {
        ...
        test('methods throw error if apiKey is missing', async () => {
            ...
            await expect(noAuthClient.v1.chatCompletions({})).rejects.toThrow('No api key!');
            await expect(noAuthClient.v1.getEmbeddings({})).rejects.toThrow('No api key!');
            await expect(noAuthClient.v1.moderations({})).rejects.toThrow('No api key!');
            await expect(noAuthClient.v1.getStats()).rejects.toThrow('No api key!');
            await expect(noAuthClient.v1.createKey({ name: 'test' })).rejects.toThrow('No api key!');
            await expect(noAuthClient.v1.deleteKey('123')).rejects.toThrow('No api key!');
        });
    });
});
